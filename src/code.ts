...
import * as fsBase from 'fs'; // Importa somente para o tipo Dirent
...
class leituraDiretorio {
    ...
    async listarDiretorio(caminho: string): Promise<fsBase.Dirent[]> {
        try{
            const itens = await fs.readdir(caminho, { withFileTypes: true });
            return itens as fsBase.Dirent[];
        } catch (error) {
    ...
    async listarArquivos(caminho:string): Promise<string[]>{
        try{
            ...
            return arquivos.filter((arquivo: fsBase.Dirent) => arquivo.isFile()).map((arquivo: fsBase.Dirent) => arquivo.name);
        } catch (error) {
    ...
    async listarDossies(caminho:string): Promise<string[]>{
        try{
            ...
            return dossies.filter((dossie: fsBase.Dirent) => dossie.isDirectory()).map((dossie: fsBase.Dirent) => dossie.name);
        } catch (error) {
...
class moverItens {
    ...
    async moverArquivo(caminhoOrigem: string, caminhoDestino: string, arquivosOrigem: string[], dossiesDestino: string[]): Promise<boolean> {
        // Verifica se tem arquivos e dossiês para processar
        if(arquivosOrigem.length === 0 || dossiesDestino.length === 0) return false;
        // Itera sobre cada dossiê de destino e filtra os arquivos correspondentes
        for(const dossieDestino of dossiesDestino){
            const arquivosCorrespondentes = arquivosOrigem.filter((arquivo: string) => {
                return arquivo.includes(dossieDestino);
            });
            if(arquivosCorrespondentes.length === 0) return false;
    ...
    async moverDossies(caminhoOrigem: string, caminhoDestino: string, dossiesOrigem: string[], dossiesDestino: string[]): Promise<boolean> {
        // Verifica se tem dossiês para processar
        if(dossiesOrigem.length === 0 || dossiesDestino.length === 0) return false;
        // Itera sobre cada dossiê de destino e filtra os dossiês correspondentes
        for (const dossieDestino of dossiesDestino){
            const dossiesCorrespondentes = dossiesOrigem.filter((dossieOrigem: string) => {
                return dossieOrigem.includes(dossieDestino);
            })
            if(dossiesCorrespondentes.length === 0) return false;
    ...
    async moverTudo(caminhoOrigem: string, caminhoDestino: string, arquivosOrigem: string[], dossiesOrigem: string[], dossiesDestino: string[]): Promise<boolean> {
...
console.log(`Arquivos origem: ${arquivosOrigem}`);

const dossiesOrigem = await leitor.listarDossies(caminhoOrigem);
console.log(`Dossiês origem: ${dossiesOrigem}`);

const dossiesDestino = await leitor.listarDossies(caminhoDestino);
console.log(`Dossiês destino: ${dossiesDestino}`);
